...
// Debug MIDI Event capture (for debug pipe streaming)
#define DEBUG_MIDI_EVENT_BUFFER_SIZE 128 // Circular buffer size
#define DEBUG_MIDI_EVENT_INTERVAL_MS 8	 // ~120 events/sec rate limit

typedef struct DebugMidiEvent
{
	BYTE Channel;	// 0-15
	BYTE EventType; // 0=NoteOff, 1=NoteOn, 2=CC, 3=PC, 4=PitchBend
	BYTE Data1;		// Note/CC number
	BYTE Data2;		// Velocity/Value
};

DebugMidiEvent DebugMidiEvents[DEBUG_MIDI_EVENT_BUFFER_SIZE];
volatile DWORD DebugMidiEventWriteHead = 0;
volatile DWORD DebugMidiEventReadHead = 0;
DWORD LastDebugMidiEventTime = 0;
